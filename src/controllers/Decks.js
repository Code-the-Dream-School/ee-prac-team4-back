...
const getAllDecks = async (req, res) => {
  ...
  try {
    ...
    const decks = await Deck.find({ isPublic: true }).sort("createdAt");

    const decksWithDetailedFlashcards = await Promise.all(
...
const getUserDecks = async (req, res) => {
  try {
    ...
    const decks = await Deck.find({ createdBy: req.user.userId }).sort(
      "createdAt",
    );
  ...
  try {
    ...
    if (!deck) {
      return res
        ...
        .json({ msg: `No deck with id ${deckId}` });
    }
